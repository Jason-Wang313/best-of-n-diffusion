...
- true action diffusion: epsilon-prediction target, DDIM, stochastic DDPM-style sampling, one-step consistency-style sampling, clean-target ablation, diffusion-internal/oracle/adversarial scorers, seed-level CI rows, measured runtime rows, positive aligned oracle gains for DDIM and DDPM, and negative real-utility change for an anti-correlated score;
- PushT benchmark: `gym_pusht/PushT-v0`, true simulator rollout utility, DDIM/DDPM/consistency samplers, oracle/value/misaligned scorers, rollout-level CSV inventory, seed-level CI rows, aligned oracle gain, low-diversity evidence, a scorer-gap failure case, and measured wall-clock runtime;
- tiered global gate: controlled sampler, learned Diffusion Policy-lite, true action diffusion, and PushT simulator evidence must all be `SUPPORTED`; toy-only evidence cannot promote a global diffusion-policy claim.
