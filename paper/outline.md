...
Best-of-`N` obeys measurable selection laws, and an Audit-Then-Sample controller can conservatively admit or reject extra diffusion trajectories. Extra samples are valuable only when sampled trajectories are sufficiently diverse, the reranker's upper score tail has audited real utility support, and denoising latency does not dominate the lower-bound utility gain.
